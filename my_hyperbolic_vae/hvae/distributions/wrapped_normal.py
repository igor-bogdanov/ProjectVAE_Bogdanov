...
class WrappedNormal(torch.distributions.Distribution):
    ...
    @property
    def scale(self):
        return functional.softplus(self._scale) if self.softplus else self._scale
    ...
    def rsample(self, sample_shape=torch.Size()):
        ...
        # Проверяет лежит ли v в касательном пространстве для self.manifold.zero
        self.manifold.assert_check_vector_on_tangent(self.manifold.zero, v)

        # Делим v на lambda. Параллельный перенос manifold.zero в self.loc. И применение expmap

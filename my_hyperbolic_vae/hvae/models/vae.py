...
from hvae.utils import get_mean_param

class VAE(nn.Module):
    def __init__(self, prior_dist, posterior_dist, likelihood_dist, enc, dec, params=None):
        super(VAE, self).__init__()
        self.pz = prior_dist # p(z)
        self.px_z = likelihood_dist # p(x|z)
        ...
        if(self.px_z == t_dist.RelaxedBernoulli):
    ...
    def generate(self, N, K):
        ...
        return mean, \
            means.view(-1, *means.size()[2:]), samples.view(-1, *samples.size()[3:])
